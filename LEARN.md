...
- **为何 prefill 不能复刻 decode 的 `max+mask` 静态化（economics 而非 feasibility）**：「seq_len 会变」不是真理由——decode 的 KV 长度也变，却用 `max_ctx+mask` 静态化了。关键三点：① **mask 不省 compute**，它先按 max shape 把 FLOP 全算出来再 `masked_fill(-inf)` 丢掉无效部分，GPU 实算的是 padding 后的完整量。② **代价几何不对称**：decode query 长恒为 1，变的 KV 轴算量 ∝`1×L`（线性，padding 浪费是 size-1 query 上一小条，≈免费）；prefill 变的是 query 长 S，attention ∝`S²`（二次，100→2048 档 = QKV/FFN 行数 20× 白算、attention 419× 白算）。③ **可枚举性 + 收益**：decode 只需枚举 batch B 的小集合（~8 档），L 由 `seq_lens` 值驱动不进 shape；prefill 的 S 连续大范围 + varlen 多序列组合不可枚举，且 prefill 本就 compute-bound（CUDA Graph 省的 launch 已被淹没≈零收益）。→ 真实系统对 prefill 走 **varlen kernel(`cu_seqlens`,不 padding) + chunked prefill + PIECEWISE graph**，反证 padding 太贵。详见 `course9/prefill_decode.md`。
...
## [2026-06-27] vLLM v1 在线服务全链路:两进程 + busy loop + 调度
**标签**: vllm, serving, async-llm, engine-core, scheduler, zmq, continuous-batching, chunked-prefill, preemption
在线服务(`vllm serve`)= **前端进程(协议/tokenize/detokenize/异步队列)** 与 **引擎进程(EngineCoreProc busy loop)** 经 **ZeroMQ + msgpack** 解耦;引擎每个 step 都重新 `schedule()`(continuous batching 本质),用 token 预算+chunked prefill+抢占+分页 KV 动态拼批喂 GPU。承接 [2026-06-02 api_server 入口]、[2026-06-XX DP 三组件]。
- **两条数据流**:**入站** HTTP→`OpenAIServingChat`(套模板+tokenize)→`InputProcessor.process_inputs`→`EngineCoreRequest`(只带 token id+采样参数,**不带原文**,文本留前端)→`AsyncMPClient`→ZMQ→引擎;**出站** `EngineCoreOutputs`→ZMQ→`AsyncLLM.output_handler`→`OutputProcessor`+`IncrementalDetokenizer`(增量解码,处理跨 token 的半个 UTF-8)→`RequestOutput`→SSE `data:{...}`。
- **进程边界为何这么切**:把网络IO/序列化与 GPU 计算拆两进程,避免 GIL 让 socket IO 阻塞推理。引擎侧两条 IO 线程(`process_input/output_sockets`)把 ZMQ 与进程内 `input_queue/output_queue` 解耦→busy loop 只跟内存队列打交道。序列化用 `msgspec.msgpack`。离线 `LLM` 类走 `LLMEngine`+`InprocClient`(同进程直调,无 ZMQ)。
- **busy loop = `EngineCore.step()` 三连**:`scheduler.schedule()` → `executor.execute_model()` → `scheduler.update_from_output()`。**每步重新 schedule** 才是 continuous batching 的实现:新请求随时插进正在 decode 的批、完成请求随时退出,批次每步动态重组。
- **`Scheduler.schedule()` 两段式(token 预算驱动)**:预算=`max_num_batched_tokens`。**段一保 RUNNING**(decode 每步 `num_new=1`,或续 prefill);**段二纳 WAITING**(首次 prefill / 抢占恢复,受 `max_num_seqs` 并发上限)。prefill 与 decode **同批混跑**,一个 `SchedulerOutput` 下发。
- **chunked prefill**:`num_new = min(剩余prefill, token_budget)`;超长 prompt 切片跨多 step 完成,`num_computed_tokens` 游标推进→长 prompt 不会一次霸占整批把别人 decode 饿死(TPOT 受控)。开关 `enable_chunked_prefill`。
- **preemption**:`allocate_slots()` 返 None(KV 块不足)→`_preempt_request`:释放全部 KV 块、`status=PREEMPTED`、**`num_computed_tokens=0`(恢复时整段重算)**、退回 waiting **队头**优先恢复。FCFS 抢最新 running,PRIORITY 抢优先级最低。对应 ModelRunner 侧 `resumed_from_preemption=True`→block table **整表替换**而非 append。
- **KV/前缀缓存**:`KVCacheManager.allocate_slots` 向 `BlockPool` 借物理块;`get_computed_blocks` 用 block hash 查 `cached_block_hash_to_block` 命中前缀→复用块跳过重算。
- **结束判定** `FinishReason`:`LENGTH/EOS` 引擎侧判,`STOP`(stop 字符串)在前端 detokenizer 侧判(stop 是文本层概念,要 detokenize 后才能匹配)。
- 教学材料:`course7/serving_pipeline.md`(端到端 8 章+速查表,姊妹篇 `serving_arch.md` 讲 ModelRunner 内部)、`course7/serving_loop_demo.py`(纯 stdlib 复刻 busy loop:实测场景一 chunked prefill 12→8+4 + continuous batching;场景二显存填满触发 1 次抢占+整段重算)。
